...
export function extractInvoiceFields(text) {
  ...
  for (const line of lines.slice(0, 12)) {
    ...
    if (/facture|invoice|date|siret|tva|échéance|operation|opération|prestation de services|description|total|iban|swift|bic|client|destinataire/i.test(line)) {
      continue;
    }
    if (/boxing\s*center|sas\s*boxing/i.test(line)) continue;
    if (/^\d|^(tel|tél|email|http|www\.|fr\d{2})/i.test(line)) continue;
    vendorName = line;
    break;
  }
