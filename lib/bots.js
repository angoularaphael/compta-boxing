...
export function isValidBotUrl(url) {
  if (!url) return false;
  try {
...
function botTimeoutMs(path) {
  const slow = 22000;
  if (path === '/api/status') return slow;
  if (path === '/api/health') return 8000;
  if (path === '/api/qr') return slow;
  if (path === '/api/start') return slow;
  if (path === '/api/stop') return 10000;
  if (path === '/api/logout') return 12000;
  return 12000;
}
...
async function botFetch(botUrl, path, { method = 'GET', body } = {}, attempt = 0) {
  ...
  try {
    ...
    const skipRetry = path === '/api/start';
...
export async function fetchBotStatus(botUrl) {
  ...
  try {
    let data;
    try {
      data = await botFetch(botUrl, '/api/health');
      if (data.connected) {
    ...
    data = await botFetch(botUrl, '/api/status');
...
export async function fetchBotAction(botUrl, action, body = {}) {
  ...
  if (action === 'start') {
    botFetch(botUrl, path, { method: 'POST', body }).catch(() => {});
