...
/** Factures visibles pour un mois : mois comptable (= mois de réception / paiement). */
  ...
  try {
    ...
    const { data, error } = await sb
      ...
      .update({
        ...
        ocr_raw: ocr.ocrRaw ? String(ocr.ocrRaw).replace(/\u0000/g, '').slice(0, 8000) : null,
      })
      .eq('id', invoiceId)
      .select('*')
      .single();
...
export async function ingestInvoiceFile({
  ...
  sourcePhone = null,
  deferOcr = false,
  accountingMonth: accountingMonthInput = null,
}) {
  const sb = getSupabase();
  const accountingMonth =
    parseAccountingMonth(accountingMonthInput) || accountingMonthFromDate(new Date());
