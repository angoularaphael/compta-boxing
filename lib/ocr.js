...
import { extractTextFromPdfStreams } from './pdf-text.js';
...
export async function extractTextFromPdf(buffer) {
  ...
  // PDFKit / xref cassé : pdf-parse renvoie vide ou échoue → fallback flux
  if (text.length < 20) {
    const fromStreams = extractTextFromPdfStreams(buffer);
