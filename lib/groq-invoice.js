...
import {
  ...
  extractPeriodOrMonthDate,
  parseFrenchAmount,
} from './extract-invoice.js';
...
const INVOICE_JSON_PROMPT = `Extrais les infos de ce justificatif français (facture, ticket de caisse, carburant ou bulletin de salaire) en JSON strict :
{
...
- Si période "Du 01/09/25 AU 30/09/25" → invoice_date = date de FIN (2025-09-30)
- Si mois+année seul (ex. "novembre 2025", "octobre 2025") → invoice_date = dernier jour du mois (2025-11-30)
- amount_ttc = Total TTC / Total T.T.C. / Net à payer — nombre sans symbole €
- BULLETIN DE SALAIRE / fiche de paie : amount_ttc = « Net à payer » / « NET A PAYER AVANT IMPOT SUR LE REVENU » (PAS le brut, PAS le net imposable, PAS les cumuls, PAS le plafond Sécu). Si « Paiement le : JJ/MM/AA » → invoice_date = cette date. vendor_name = nom du salarié.
- vendor_name = ÉMETTEUR (celui qui facture / doit être payé), JAMAIS le client — sauf fiche de paie (salarié)
- BOXING CENTER, SAS BOXING CENTER, MINIMES, RAMONVILLE, SAINT CYPRIEN = CLIENT → pas vendor_name (sauf si c'est l'émetteur d'une FAC/DEV)
- Ex. "Fanny LALOGE" + "SAS BOXING CENTER MINIMES" → vendor_name = "Fanny LALOGE"
- invoice_number = n° facture / ticket (ex. "766", "2026-06-1", "FAC-2026-003", "F00007") ; sinon null
...
/** Texte PDF chiffré / binaire (ex. Shape Coaching) → inutilisable. */
function isGarbagePdfText(text) {
  const t = String(text || '').trim();
  if (t.length < 20) return true;
  const letters = (t.match(/[A-Za-zÀ-ÿ]/g) || []).length;
  const ratio = letters / t.length;
  if (ratio < 0.35) return true;
  if (!/\d{1,2}[/.-]\d{1,2}|total|facture|€|\d+[,.]\d{2}/i.test(t) && ratio < 0.55) {
    return true;
  }
  return false;
}
...
function normalizeGroqResult(parsed, rawText = '', fileName = '') {
  let invoiceDate = parseIsoDate(parsed?.invoice_date);
  if (!invoiceDate) {
    invoiceDate = extractPeriodOrMonthDate(rawText);
  }
  const amountTtc = parseFrenchAmount(parsed?.amount_ttc);
  let vendorName = correctVendorName(
    parsed?.vendor_name ? String(parsed.vendor_name).trim().slice(0, 200) : null,
    rawText
  );
  if (!vendorName || vendorName.toLowerCase() === 'null' || /^non\s*visible$/i.test(vendorName)) {
    vendorName = null;
  }
  // OCR binaire / PDF chiffré → rejeter les « noms » illisibles
  if (vendorName) {
    const letters = (vendorName.match(/[A-Za-zÀ-ÿ]/g) || []).length;
    if (letters < 3 || letters / vendorName.length < 0.45) vendorName = null;
  }
...
async function analyzeTextWithGroq(text, fileName) {
  ...
  for (const model of models) {
    try {
      ...
      const result = enrichPartialWithLocal(normalizeGroqResult(parsed, clean, fileName), clean, fileName);
      if (result.ocrStatus === 'ok') return result;
      if (result.ocrStatus === 'partial') bestPartial = result;
    } catch (err) {
  ...
  const fallback = enrichPartialWithLocal(localTextFallback(clean, fileName), clean, fileName);
  if (fallback.ocrStatus !== 'failed') {
    return { ...fallback, analyzer: fallback.analyzer === 'groq' ? 'local-text' : fallback.analyzer };
  }
  if (bestPartial) return bestPartial;
  throw new Error('Analyse texte Groq + local échouée');
}

function enrichPartialWithLocal(result, clean, fileName) {
  if (!result) return result;

  const local = extractInvoiceFields(clean);
  const isPayslip = local.documentKind === 'payslip' || /bulletin\s+de\s+salaire/i.test(clean);

  // Fiches de paie : le parseur local (net à payer) prime sur Groq qui prend souvent le brut/max
  let invoiceDate = result.invoiceDate || local.invoiceDate || extractPeriodOrMonthDate(clean);
  let amountTtc = result.amountTtc != null ? result.amountTtc : local.amountTtc;
  let vendorName = result.vendorName || local.vendorName || null;

  if (isPayslip) {
    if (local.amountTtc != null) amountTtc = local.amountTtc;
    if (local.invoiceDate) invoiceDate = local.invoiceDate;
    if (local.vendorName) vendorName = local.vendorName;
  } else if (result.invoiceDate && result.amountTtc != null && result.ocrStatus === 'ok') {
    return result;
  }

  const invoiceNumber =
    result.invoiceNumber || local.invoiceNumber || extractInvoiceNumberFromFileName(fileName);
...
export async function analyzeInvoiceWithGroq(buffer, mimeType, fileName = '') {
  ...
  if (isPdf) {
    ...
    if (text && text.length >= 20 && !isGarbagePdfText(text)) {
    ...
    if (jpeg) {
      return analyzeImageBufferWithGroq(
        ...
        'image/jpeg',
        fileName,
        'Document scanné ou PDF image. Peut être une facture ou un ticket carburant.'
      );
    }

    // Dernier recours : tenter le texte même médiocre (parfois partiellement lisible)
