...
- invoice_number = n° facture / ticket (ex. "766", "2026-06-1", "F00007") ; sinon null
...
function enrichPartialWithLocal(result, clean, fileName) {
  ...
  if (result.invoiceDate && result.amountTtc != null && result.ocrStatus === 'ok') return result;

  const local = extractInvoiceFields(clean);
  const invoiceDate = result.invoiceDate || local.invoiceDate || extractPeriodOrMonthDate(clean);
  const amountTtc = result.amountTtc != null ? result.amountTtc : local.amountTtc;
  const vendorName = result.vendorName || local.vendorName || null;
  const invoiceNumber =
