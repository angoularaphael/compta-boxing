...
async function bufferToPdfLibImage(merged, buf, mimeType) {
  const lower = String(mimeType || '').toLowerCase();
  if (lower === 'image/png') return merged.embedPng(buf);
  if (lower === 'image/jpeg' || lower === 'image/jpg') return merged.embedJpg(buf);

  // webp / heic / etc. → tenter conversion via sharp (souvent dispo avec Next)
  if (lower === 'image/webp' || lower === 'image/heic' || lower === 'image/heif') {
    try {
      const sharp = (await import('sharp')).default;
      const png = await sharp(buf).png().toBuffer();
      return merged.embedPng(png);
    } catch {
      return null;
    }
  }
...
async function appendImagePage(merged, buf, mimeType) {
  const image = await bufferToPdfLibImage(merged, buf, mimeType);
