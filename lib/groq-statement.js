...
const STATEMENT_JSON_PROMPT = `Tu analyses un relevé bancaire français Banque Populaire (PDF texte extrait).
Extrais UNIQUEMENT les dépenses / débits (argent qui sort du compte).
Sur ce format : les débits ont un tiret avant le montant (« - 163,20 € ») ; les crédits n'en ont pas → ignore-les.
Les lignes « SOLDE CREDITEUR AU JJ/MM/AAAA … € » sont des soldes (ouverture/clôture), PAS des opérations → ignore-les et continue à parser.
...
- date ISO ; formats FR DD/MM convertis avec l'année du relevé
- cartes CB, PRLV SEPA, VIR INST, EUROVIR, chèques, frais = dépenses
- CONSERVE les doublons (même date + même montant = plusieurs lignes si présentes)
- une ligne par opération débit trouvée`
